...
valueBox(
  ...
  caption = 
    paste0(
      "This section aims to analyze the relationship between COVID-19 case fatality rates (CFR) and various socioeconomics indictors. Relevant demographic and socioeconomic data include indicators such as total population, population density, GDP, GNI, life expectancy, birth rate, death rate, population growth, urban population (%), and rural population (%).", 
      "<br>",
      "<br>",
      "The extracted data was organized into comprehensive dataframe covering all 12 countries. Two versions of the dataframe were used for analysis: (1) a dataframe consisting of the average value of each indicator from 2020 to 2022 for each country and (2) a graded dataframe of the indicators. The graded dataframe consisted of dividing the values of each indicator into quantiles, creating thresholds categorized as \"Very Low\", \"Low\", \"Medium\", \"High\", or \"Very High\" based on their respective indicator values.",
       "<br>",
      "<br>",
      "You can interact with these plots by hovering over the chart lines provides precise numerical values for each metric."),
  color = "#374785"
)
```
...
valueBox(
  ...
  caption = 
    paste0(
      "A linear regression model was used to find a strongest predictor against the (1) a dataframe consisting of the average value of each indicator from 2020 to 2022 for each country and (2) a graded dataframe of the indicators.", 
      "<br>",
      "<br>",
      "The (2) linear regression model had a low sample size for each grade, leading to high multicollinearity. It had low varaition within groups and no residual degrees of freedom. Therefore, a linear regression model was tested on (1) and the following results is shown below. The PGP (population growth) has the largest coefficient magnitude, although the significance was unable to be calculated. Another analysis is required."),
  color = "#084b83"
)
```
...
valueBox(
  ...
  caption = 
    paste0(
      "Using the first (1) dataframe consisting of the average value of each indicator (continuous)from 2020 to 2022 for each country, a correlation analysis was done against all the predictor and response variables as shown below. Life expectancy has the largest magnitude correlation with average CFR, with population density as a close second."),
  color = "#084b83"
)
```
